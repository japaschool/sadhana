...
pub fn prompt(props: &Props) -> Html {
    ...
    html! {
        <div class="fixed top-0 w-full h-screen z-20">
            <div
                class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity overflow-hidden"
            />
            <div class="fixed inset-0 w-screen overflow-y-auto">
                <form class="flex min-h-screen justify-center p-4 text-center items-center sm:p-0">
                    <div
                        class={tw_merge!("relative transform overflow-hidden rounded-lg text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg", POPUP_BG_CSS)}
                    >
                        ...
                        <div
                            class="px-4 py-3 flex flex-col space-y-4 md:space-y-0 md:flex-row-reverse md:space-x-3 md:space-x-reverse"
                        >
                            <button
                                type="reset"
