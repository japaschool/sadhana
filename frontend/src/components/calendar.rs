...
pub fn calendar(props: &Props) -> Html {
    ...
    let next_week_onclick = {
        ...
        Callback::from(move |_: MouseEvent| {
            if selected_date.weekday() == Weekday::Sun {
                selected_date.set(selected_date.succ_opt().unwrap());
            } else {
    ...
    let prev_week_onclick = {
        ...
        Callback::from(move |_: MouseEvent| {
            if selected_date.weekday() == Weekday::Mon {
                selected_date.set(selected_date.pred_opt().unwrap());
            } else {
    ...
    const HOVER_DATE_DIV_CSS: &'static str = 
        "flex group hover:bg-zinc-300 dark:hover:bg-slate-800 rounded-full mt-2 mx-1 transition-all duration-300 cursor-pointer justify-center h-8 w-8";
    const SELECTED_TODAY_DATE_DIV_CSS: &'static str = 
        "flex group text-white bg-amber-400 rounded-full mt-2 mx-1 cursor-pointer justify-center h-9 w-9";
    const SELECTED_DATE_DIV_CSS: &'static str = 
        "flex group text-white rounded-full border-2 border-amber-400 mt-2 mx-1 cursor-pointer justify-center h-9 w-9";
